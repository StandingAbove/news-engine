"""Small MLP baseline (PyTorch).

Takes the last `lookback` observations, predicts the next one. Forecasting
the full horizon is done recursively (feed prediction back as input).
Deliberately tiny — three hidden layers, 64 units — because the point is
to have a learnable baseline, not to win on its own merits.
"""
...
class MLPForecaster:
    ...
    def fit(self, history: np.ndarray) -> None:
        ...
        # Standardize so the net doesn't have to learn scale.
